...
4. **运行时验证的 cwd 陷阱**：产物里 `external_ui: "ui"` 与 `cache_file.path:
   "cache.db"` 都是相对路径，sing-box 按进程 cwd 解析。本机宿主起实例验证时
   cwd=仓库根，Yacd 静态文件（1.8MB）与 cache.db 直接落进仓库。规则同沙箱：
   运行时验证必须在临时目录（cwd 指向它）或 VM 内执行。
