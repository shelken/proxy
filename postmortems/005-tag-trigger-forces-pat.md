...
**选错的是触发点**：把 tag 当成「跨工作流的信号」。一旦试图由上游工作流通过 `GITHUB_TOKEN` 推送 tag 来唤醒下游工作流，就会撞上官方防递归事件抑制；当时误以为必须引入 PAT，其实真正的问题在于跨工作流触发本身。

**核心机制**：GitHub Actions 的事件抑制**只约束自动化凭据（如 `GITHUB_TOKEN`）产生的文件或 tag 推送**；**人类在 GitHub 网页端点击 Merge 或本地推送所产生的事件，天生正常触发下游工作流，不受任何抑制**。因此，将发布触发点收敛至维护者合并 Release PR（`push: main`），并在该 run 内部完成打 tag、编译与发布，全程闭环，天然不需要任何 PAT。
